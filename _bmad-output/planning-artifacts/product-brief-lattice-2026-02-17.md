...
lattice provides two distinct modes of intelligence for agent teams:
...
2. **Agent-Native Skill Design** — Built as an MCP server from the ground up, lattice feels like a native agent capability, not an external service bolted on.
...
**Profile:** Any MCP-compatible AI agent operating within a development team — Claude Code, OpenCode, Cursor, Windsurf, or custom orchestration layers. These agents are the high-frequency consumers of lattice, making dozens to hundreds of tool calls per session.
...
- Confidence that the lattice toolchain is reliable, recoverable, and low-maintenance
...
**Success Looks Like:**
- Deploys lattice once, agents across the team immediately gain spec intelligence
- Reviews `graph://overview` to see the health of the entire spec landscape
- Trusts that `lattice rebuild` from git fully recovers everything — zero lock-in risk
...
|---|---|---|---|
| **Adoption** | — | Adds lattice to MCP config | Evaluates lattice, decides to standardize for the team |
...
| **Onboarding** | Discovers lattice tools via MCP | Runs first `lattice sync` to build index | Reviews `graph://overview` to validate initial graph |
...
lattice is an open-source experiment in standardizing specification-driven development for AI agent teams. Success is measured across three horizons: proving the technology works, proving agents adopt it, and proving the concept spreads.
...
- `lattice rebuild` from git produces identical results — zero data anxiety
...
| **Prove agent adoption** | Agent teams actively using lattice in real workflows | 6 months |
| **Prove community interest** | Meaningful GitHub traction and external contributors | 12 months |
| **Prove the standard** | Other projects adopt spec-driven development patterns inspired by lattice | 18+ months |
...
- Rebuild reliability: `git clone` + `lattice rebuild` produces identical index — 100% idempotent
- Sync speed: Incremental sync processes only changed files via `git diff` in < 5 seconds

**Adoption KPIs (P1 — Do agents use it?)**
- Agent tool call frequency: lattice MCP tools become preferred over file-reading for spec context
- Causal edge growth: AI-inferred edges accumulate and a meaningful percentage survive human review
- Spec evolution velocity: teams with lattice iterate on specs faster with complete traceability
...
- Ecosystem influence: Other tools or frameworks reference or integrate lattice patterns
...
| **Git Sync Engine** | Bidirectional sync between git repository and lattice indexes — full rebuild from tree walk and incremental sync via `git diff` |
...
| **OpenTelemetry Instrumentation** | Traces and metrics for all lattice operations — search latency, graph traversal time, sync duration, MCP tool call frequency. Enables teams to observe agent usage patterns and validate adoption KPIs with real data |
...
| **Edge Export to Git** (`.lattice/edges.yaml`) | Only needed when AI-inferred edges exist to review | P2 |
...
| **Git sync is reliable** | `lattice rebuild` from clean git clone produces identical index | Must pass |
| **Incremental sync works** | Changed specs re-indexed without full rebuild via `git diff` | Must pass |
| **MCP integration works** | AI agent (Claude Code / OpenCode) successfully discovers and calls lattice tools | Must pass |
| **Observability works** | OpenTelemetry traces and metrics emitted for all key operations | Must pass |
| **Real-world validation** | lattice used to manage its own specifications during development (dogfooding) | Should pass |
...
- AI-inferred edges periodically exported to `.lattice/edges.yaml` in git for human review
- The knowledge graph becomes a self-improving system — each agent interaction makes it smarter

**Long-Term Vision**
- lattice becomes the standard specification management layer for AI agent teams
...
- Spec-driven development emerges as a recognized engineering discipline — lattice is the reference implementation

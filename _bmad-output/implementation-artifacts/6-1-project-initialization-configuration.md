...
- [x] Implement initialization service in `src/main.rs` (or `src/init.rs` if extracted)
  ...
  - [x] Ensure directories exist: `specs/`, `.lattice/`, `data/tantivy/`, `data/fjall/`
  - [x] Guard existing `.lattice/config.yaml` and return warning without overwrite
...
- [x] Emit clear init UX
  - [x] Print created paths and next-step commands (`lattice sync`, `lattice serve`, `lattice status`)
  - [x] Print non-destructive warning when config exists
- [x] Add tests
  - [x] Unit test defaults + merge semantics for optional fields
  - [x] Integration test `lattice init` in empty temp dir
  - [x] Integration test `lattice init` idempotency when config already exists
...
- Generated runtime/config tree must match architecture layout: `specs/`, `.lattice/config.yaml`, `data/tantivy/`, `data/fjall/`.
...
- Added `lattice init` CLI scaffolding with idempotent config guard and printed next-step guidance.
